...
tabledf1 = tabledf1.map(lambda line:(line[0],line[1],line[2][0],line[2][1])).filter(lambda line: len(line)==4)

#Call an action to execute the job
print('The total number of rows is:',tabledf1.count())
#print(tabledf2.count())

tableDF = spark.createDataFrame(tabledf1)

tableDF.write.format('csv').option('header',True).mode('overwrite').save('file:///F:/Hadoop/sparkjobtest.csv')
